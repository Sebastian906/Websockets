...
/**
 * Fetch wrapper with simple retry/backoff for transient network errors
 * Retries on thrown fetch errors and on 5xx or 429 HTTP responses.
 */
async function fetchWithRetry(url, options = {}, attempts = 5, baseDelay = 200) {
    let lastErr = null;
    for (let i = 0; i < attempts; i += 1) {
        ...
        if (i < attempts - 1) {
            const delay = Math.min(5000, baseDelay * 2 ** i) + Math.floor(Math.random() * 100);
...
async function fetchMatches(limit = 100) {
    const response = await fetchWithRetry(`${API_URL}/matches?limit=${limit}`, { method: 'GET' }, 5);
